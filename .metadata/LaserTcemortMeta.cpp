...
void LaserTcemort::Load(const std::string &filepath)
{
	bytes::ToBytes bytes;
	bytes.Load(filepath, _speed, _damage);
}

void LaserTcemort::Save(const std::string &filepath)
{
	bytes::ToBytes bytes;
	bytes.Save(filepath, _speed, _damage);
}

std::map<std::string, metadata_t> LaserTcemort::GetMetadata()
{
