...
void BlockSpawner::Load(const std::string &filepath)
{
	bytes::ToBytes bytes;
	bytes.Load(filepath, _spawnDelay, _prefabName);
}

void BlockSpawner::Save(const std::string &filepath)
{
	bytes::ToBytes bytes;
	bytes.Save(filepath, _spawnDelay, _prefabName);
}

std::map<std::string, metadata_t> BlockSpawner::GetMetadata()
{
