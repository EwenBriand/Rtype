...
{
	bytes::ToBytes bytes;
	bytes.Load(filepath, _tag, _collidedTag, _layer, _vertices);
}

void Collider2D::Save(const std::string &filepath)
{
	bytes::ToBytes bytes;
	bytes.Save(filepath, _tag, _collidedTag, _layer, _vertices);
}
...
std::map<std::string, metadata_t> Collider2D::GetMetadata()
{
	...
	metadata["_collidedTag"] = {"std::string", "_collidedTag", sizeof(std::string), [&](const std::string &str) {_collidedTag = fromString<decltype(_collidedTag)>(str);}, [&]() { return &_collidedTag; }, [&]() { std::ostringstream oss;oss << _collidedTag;return oss.str();}};
