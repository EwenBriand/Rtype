...
bool InputManager::isDown(const int key_code)
{
    for (const auto& event : m_polledEvents) {
        if (event->key_code == key_code && event->status) {
            return event->status;
        }
    }
    return false;
...
bool InputManager::isReleased(const int key_code)
{

    for (const auto& event : m_polledEvents) {
        if (event->key_code == key_code && IsKeyReleased(event->key_code)) {
            return true;
        }
    }
    return false;
