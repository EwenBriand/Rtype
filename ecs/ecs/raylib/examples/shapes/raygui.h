...
*       - Standalone mode needs library modification and some user work to plug another backend
*
*   CONTROLS PROVIDED:
...
*       needs 8 integers (16*16/32) to be stored in memory.
*
...
// NOTE: Microsoft specifiers to tell compiler that symbols are imported/serializeed from a .dll
        ...
        #define RAYGUIAPI __declspec(dllserialize)     // We are building the library as a Win32 shared library (.dll)
...
// every 16x16 icon needs 8 integers (16*16/32) to be stored
//
...
int GuiTextBox(Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            // we add an index offset to text for drawing only what needs depending on cursor
...
// NOTE: Tooltips needs some global variables: tooltipPtr
//----------------------------------------------------------------------------------
...
static void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
        ...
        (signature[3] == ' '))
    {
        ...
        if (fontDataSize > 0)
        {
            ...
            {
                // Compressed font atlas image data (DEFLATE), it needs DecompressData()
...
static void GuiDrawText(const char *text, Rectangle bounds, int alignment, Color tint)
{
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        for (int i = 0; i < lineCount; i++)
        {
            ...
            // If text needs an icon, add size to measure
            if (iconId >= 0)
            {
