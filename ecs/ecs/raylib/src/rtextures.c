...
bool ExportImage(Image image, const char *fileName)
{
    ...
    if (success != 0) TRACELOG(LOG_INFO, "FILEIO: [%s] Image serializeed successfully", fileName);
    else TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to serialize image", fileName);
...
bool ExportImageAsCode(Image image, const char *fileName)
{
    ...
    if (success != 0) TRACELOG(LOG_INFO, "FILEIO: [%s] Image as code serializeed successfully", fileName);
    else TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to serialize image as code", fileName);
