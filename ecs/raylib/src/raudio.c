...
bool ExportWave(Wave wave, const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".qoa"))
    {
        ...
        else TRACELOG(LOG_WARNING, "AUDIO: Wave data must be 16 bit per sample for QOA format serialize");
    }
#endif
    ...
    if (success) TRACELOG(LOG_INFO, "FILEIO: [%s] Wave data serializeed successfully", fileName);
    else TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to serialize wave data", fileName);
...
bool ExportWaveAsCode(Wave wave, const char *fileName)
{
    ...
    byteCount += sprintf(txtData + byteCount, "//                                                                              //\n");
    byteCount += sprintf(txtData + byteCount, "// WaveAsCode serializeer v1.1 - Wave data serializeed as an array of bytes           //\n");
    byteCount += sprintf(txtData + byteCount, "//                                                                              //\n");
    ...
    // Wave data is serializeed as byte array for 8/16bit and float array for 32bit float data
    // NOTE: Frame data serializeed is channel-interlaced: frame01[sampleChannel1, sampleChannel2, ...], frame02[], frame03[]
    ...
    if (success != 0) TRACELOG(LOG_INFO, "FILEIO: [%s] Wave as code serializeed successfully", fileName);
    else TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to serialize wave as code", fileName);
...
// Check if any audio stream buffers needs refill
