 ...
 *           Support image serialize in multiple file formats
 *
...
#if (defined(SUPPORT_FILEFORMAT_BMP) || defined(SUPPORT_FILEFORMAT_PNG) || defined(SUPPORT_FILEFORMAT_TGA) || defined(SUPPORT_FILEFORMAT_JPG) || defined(SUPPORT_FILEFORMAT_PSD) || defined(SUPPORT_FILEFORMAT_GIF) || defined(SUPPORT_FILEFORMAT_HDR) || defined(SUPPORT_FILEFORMAT_PIC) || defined(SUPPORT_FILEFORMAT_PNM))
...
#if (defined(SUPPORT_FILEFORMAT_DDS) || defined(SUPPORT_FILEFORMAT_PKM) || defined(SUPPORT_FILEFORMAT_KTX) || defined(SUPPORT_FILEFORMAT_PVR) || defined(SUPPORT_FILEFORMAT_ASTC))
...
#if defined(SUPPORT_FILEFORMAT_PNG) || defined(SUPPORT_FILEFORMAT_BMP) || defined(SUPPORT_FILEFORMAT_TGA) || defined(SUPPORT_FILEFORMAT_JPG) || defined(SUPPORT_FILEFORMAT_GIF) || defined(SUPPORT_FILEFORMAT_PIC) || defined(SUPPORT_FILEFORMAT_HDR) || defined(SUPPORT_FILEFORMAT_PNM) || defined(SUPPORT_FILEFORMAT_PSD)
...
bool IsImageReady(Image image)
{
    return ((image.data != NULL) && // Validate pixel data available
        (image.width > 0) && (image.height > 0) && // Validate image size
...
bool ExportImage(Image image, const char* fileName)
{
    ...
    if (false) {
...
#endif
#if defined(SUPPORT_FILEFORMAT_JPG)
    else if (IsFileExtension(fileName, ".jpg") || IsFileExtension(fileName, ".jpeg"))
        ...
        if (image.format == PIXELFORMAT_UNCOMPRESSED_R8G8B8)
            ...
            TRACELOG(LOG_WARNING, "IMAGE: Image pixel format must be R8G8B8 or R8G8B8A8");
    ...
    if (success != 0)
        TRACELOG(LOG_INFO, "FILEIO: [%s] Image serializeed successfully", fileName);
    else
        TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to serialize image", fileName);
...
#define TEXT_BYTES_PER_LINE 20
    ...
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "// ImageAsCode serializeer v1.0 - Image pixel data serializeed as an array of bytes         //\n");
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    ...
    // NOTE: Text data size serializeed is determined by '\0' (NULL) character
    ...
    if (success != 0)
        TRACELOG(LOG_INFO, "FILEIO: [%s] Image as code serializeed successfully", fileName);
    else
        TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to serialize image as code", fileName);
...
Image ImageText(const char* text, int fontSize, Color color)
{
    ...
    TRACELOG(LOG_WARNING, "IMAGE: ImageTextEx() needs module: rtext");
#endif
    return imText;
}
...
void ImageResize(Image* image, int newWidth, int newHeight)
{
    ...
    if ((image->format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) || (image->format == PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) || (image->format == PIXELFORMAT_UNCOMPRESSED_R8G8B8) || (image->format == PIXELFORMAT_UNCOMPRESSED_R8G8B8A8)) {
        int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
        unsigned char* output = (unsigned char*)RL_MALLOC(newWidth * newHeight * bytesPerPixel);

        switch (image->format) {
        case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
            stbir_resize_uint8((unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 1);
            break;
        case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
            stbir_resize_uint8((unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 2);
            break;
        case PIXELFORMAT_UNCOMPRESSED_R8G8B8:
            stbir_resize_uint8((unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 3);
            break;
        case PIXELFORMAT_UNCOMPRESSED_R8G8B8A8:
            stbir_resize_uint8((unsigned char*)image->data, image->width, image->height, 0, output, newWidth, newHeight, 0, 4);
            break;
        default:
            break;
        }
        ...
        for (int y = 0; y < image->height; y++) {
            for (int x = 0; x < image->width; x++) {
                ...
                for (int i = 0; i < bytesPerPixel; i++)
                    flippedData[(y * image->width + x) * bytesPerPixel + i] = ((unsigned char*)image->data)[(y * image->width + (image->width - 1 - x)) * bytesPerPixel + i];
            }
        }
    ...
    if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
        ...
        for (int y = 0; y < image->height; y++) {
            for (int x = 0; x < image->width; x++) {
                ...
                for (int i = 0; i < bytesPerPixel; i++)
                    rotatedData[(x * image->height + (image->height - y - 1)) * bytesPerPixel + i] = ((unsigned char*)image->data)[(y * image->width + x) * bytesPerPixel + i];
            }
        }
    ...
    if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
        ...
        for (int y = 0; y < image->height; y++) {
            for (int x = 0; x < image->width; x++) {
                ...
                for (int i = 0; i < bytesPerPixel; i++)
                    rotatedData[(x * image->height + y) * bytesPerPixel + i] = ((unsigned char*)image->data)[(y * image->width + (image->width - x - 1)) * bytesPerPixel + i];
            }
        }
...
void ImageColorReplace(Image* image, Color color, Color replace)
{
    ...
    for (int y = 0; y < image->height; y++) {
        for (int x = 0; x < image->width; x++) {
            if ((pixels[y * image->width + x].r == color.r) && (pixels[y * image->width + x].g == color.g) && (pixels[y * image->width + x].b == color.b) && (pixels[y * image->width + x].a == color.a)) {
...
Color* LoadImageColors(Image image)
{
    ...
    if (image.format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
        ...
        if ((image.format == PIXELFORMAT_UNCOMPRESSED_R32) || (image.format == PIXELFORMAT_UNCOMPRESSED_R32G32B32) || (image.format == PIXELFORMAT_UNCOMPRESSED_R32G32B32A32))
...
#else
    TRACELOG(LOG_WARNING, "IMAGE: ImageDrawText() needs module: rtext");
#endif
}
...
Color GetPixelColor(void* srcPtr, int format)
{
    ...
    case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
        ...
        break;
    case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
        color = (Color) { ((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[1] };
        break;
    case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
    ...
    case PIXELFORMAT_UNCOMPRESSED_R8G8B8A8:
        color = (Color) { ((unsigned char*)srcPtr)[0], ((unsigned char*)srcPtr)[1], ((unsigned char*)srcPtr)[2], ((unsigned char*)srcPtr)[3] };
        break;
    case PIXELFORMAT_UNCOMPRESSED_R8G8B8:
