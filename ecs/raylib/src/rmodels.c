...
bool ExportMesh(Mesh mesh, const char* fileName)
{
    ...
    if (IsFileExtension(fileName, ".obj")) {
        ...
        int dataSize = mesh.vertexCount * (int)strlen("v 0000.00f 0000.00f 0000.00f") + mesh.vertexCount * (int)strlen("vt 0.000f 0.00f") + mesh.vertexCount * (int)strlen("vn 0.000f 0.00f 0.00f") + mesh.triangleCount * (int)strlen("f 00000/00000/00000 00000/00000/00000 00000/00000/00000");
        ...
        byteCount += sprintf(txtData + byteCount, "# //                                                                              //\n");
        byteCount += sprintf(txtData + byteCount, "# // rMeshOBJ serializeer v1.0 - Mesh serializeed as triangle faces and not optimized   //\n");
        byteCount += sprintf(txtData + byteCount, "# //                                                                              //\n");
        ...
        // NOTE: Text data length serializeed is determined by '\0' (NULL) character
        success = SaveFileText(fileName, txtData);

        RL_FREE(txtData);
    } else if (IsFileExtension(fileName, ".raw")) {
        // TODO: Support additional file formats to serialize mesh vertex data
    }
...
void GenMeshTangents(Mesh* mesh)
{
    if ((mesh->vertices == NULL) || (mesh->texcoords == NULL)) {
        TRACELOG(LOG_WARNING, "MESH: Tangents generation needs texcoord vertex attribute data");
        return;
    }
...
static Image LoadImageFromCgltfImage(cgltf_image* cgltfImage, const char* texPath)
{
    ...
    {
        if ((strlen(cgltfImage->uri) > 5) && (cgltfImage->uri[0] == 'd') && (cgltfImage->uri[1] == 'a') && (cgltfImage->uri[2] == 't') && (cgltfImage->uri[3] == 'a') && (cgltfImage->uri[4] == ':')) // Check if image is provided as base64 text data
        {
...
#define LOAD_ATTRIBUTE(accesor, numComp, dataType, dstPtr)                                                                                                       \
    ...
    if (result == cgltf_result_success) {
        ...
        // NOTE: We keep an extra slot for default material, in case some mesh needs it
        ...
        //  - Only supports linear interpolation (default method in Blender when checked "Always Sample Animations" when serializeing a GLTF file)
                ...
                for (unsigned int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++) {
                    ...
                    if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_joints) // JOINTS_n (vec4: 4 bones max per vertex / u8, u16)
                    {
                        ...
                        if ((attribute->component_type == cgltf_component_type_r_8u) && (attribute->type == cgltf_type_vec4)) {
                            ...
                            TRACELOG(LOG_WARNING, "MODEL: [%s] Joint attribute data format not supported, use vec4 u8", fileName);
    ...
    // WARNING: cgltf needs the file pointer available while reading data
