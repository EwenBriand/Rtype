...
/*
** @file Force_ic.cpp
** @brief Implementation of the Force_ic class.
...
MANAGED_RESOURCE(Force_ic)

// ===========================================================================================================
// Component
// ===========================================================================================================

void Force_ic::OnAddComponent(int entityID)
{
...
void Force_ic::Start()
{
    std::cout << "Force_ic::Start()" << std::endl;
    ...
    _collider->SetOnCollisionEnter([this](int entityID, int otherID) {
        try {
            ...
            if (tag.compare(0, 6, "player") == 0) {
                ...
                std::cout << "Force_ic::OnCollisionEnter(): " << tag << std::endl;
            }
...
void Force_ic::Update(int entityID)
{
    ...
    try {
        if ((_core->x < -100 or _core->x > 1920 + 100 or _core->y < -100 or _core->y > 1080 + 100)) {
            SYS.UnregisterEntity(_entity);
            std::cout << "Force_ic::Update(): out of bounds" << std::endl;
        }
    } catch (std::exception& e) {
...
void Force_ic::broadcastPosition()
{
