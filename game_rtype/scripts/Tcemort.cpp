...
/*
** @file Tcemort.cpp
** @brief Implementation of the Tcemort class.
...
MANAGED_RESOURCE(Tcemort)

// ===========================================================================================================
// Component
// ===========================================================================================================

void Tcemort::OnAddComponent(int entityID)
{
...
void Tcemort::Start()
{
    std::cout << "Tcemort::Start()" << std::endl;
    ...
    _collider->SetOnCollisionEnter([this](int entityID, int otherID) {
        try {
            ...
            if (tag.compare(0, 6, "player") == 0) {
                ...
                std::cout << "Tcemort::OnCollisionEnter(): " << tag << std::endl;
            }
...
void Tcemort::Update(int entityID)
{
    ...
    try {
        if ((_core->x < -100 or _core->x > 1920 + 100 or _core->y < -100 or _core->y > 1080 + 100)) {
            SYS.UnregisterEntity(_entity);
            std::cout << "Tcemort::Update(): out of bounds" << std::endl;
        }
    } catch (std::exception& e) {
...
void Tcemort::broadcastPosition()
{
