...
/*
** @file X2.cpp
** @brief Implementation of the X2 class.
...
void X2::OnAddComponent(int entityID)
{
    ...
    _collider->SetOnCollisionEnter([this](int entityID, int otherID) {
        try {
            ...
            if (tag.compare(0, 6, "player") == 0) {
                ...
                std::cout << "X2::OnCollisionEnter(): " << tag << std::endl;
            }
...
void X2::Update(int entityID)
{
    ...
    try {
        if ((_core->x < -100 or _core->x > 1920 + 100 or _core->y < -100 or _core->y > 1080 + 100)) {
            SYS.UnregisterEntity(_entity);
            std::cout << "X2::Update(): out of bounds" << std::endl;
        }
    } catch (std::exception& e) {
