...
/*
** @file X3.cpp
** @brief Implementation of the X3 class.
...
void X3::OnAddComponent(int entityID)
{
    ...
    _collider->SetOnCollisionEnter([this](int entityID, int otherID) {
        try {
            ...
            if (tag.compare(0, 6, "player") == 0) {
                ...
                std::cout << "X3::OnCollisionEnter(): " << tag << std::endl;
            }
...
void X3::Update(int entityID)
{
    ...
    try {
        if ((_core->x < -100 or _core->x > 1920 + 100 or _core->y < -100 or _core->y > 1080 + 100)) {
            SYS.UnregisterEntity(_entity);
            std::cout << "X3::Update(): out of bounds" << std::endl;
        }
    } catch (std::exception& e) {
